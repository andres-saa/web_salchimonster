...
class Order2:
    ...
    def create_order(self, order_data: OrderSchemaPostDistri):
        ...
        if self.can_place_order(user_id):
            ...
            self.update_order_status(order_id, order_data.payment_method_id,order_data.inserted_by, order_data.site_id)
    ...
    def procesar_carrito(self, cart):
        def calculate_total_product(product):
            ...
            # Determinar el precio unitario efectivo y el tipo de precio usado
            if pedido_cantidad >= 1000:
                ...
                product["pedido_precio"] = "distribuidor"
            elif pedido_cantidad >= 500:
                effective_unit_price = int(float(product.get("mayor", pedido_base_price)))
                discount_unit = pedido_base_price - effective_unit_price
                product["pedido_precio"] = "mayor"
            else:
                # Si no cae en mayor o distribuidor, se asume el precio menor (o normal)
                effective_unit_price = int(float(product.get("pedido_precio", pedido_base_price)))
                discount_unit = 0
                product["pedido_precio"] = "menor"  # Puedes nombrarlo como "minor" o "normal"

            # Calcular el descuento total: diferencia por unidad * cantidad + monto descuento de delivery (si existe)
            delivery_descuento = int(float(product.get("delivery_montodescuento", 0) or 0))
            total_discount = (discount_unit * pedido_cantidad) + delivery_descuento

            # Total para el producto: (precio unitario efectivo + adiciones) * cantidad
            total_producto = (effective_unit_price + adiciones) * pedido_cantidad

            # Guardamos en el dict: precio usado, subtotal y descuento calculado
            product["pedido_precio"] = effective_unit_price
            product["pedido_base_price"] = effective_unit_price
            product["pedido_subtotal"] = total_producto
            product["pedido_descuento"] = total_discount
        ...
        total_carrito = 0
        for product in cart:
            # Calcula y asigna dentro del producto todo lo necesario
            total_producto = calculate_total_product(product)
            total_carrito += total_producto

        return {
            "carro": cart,
            "total": int(total_carrito)  # Aseguramos que el total sea entero
        }
    ...
    def change_delivery_price(self, order_id: int, new_delivery_price: float):
        """
        Actualiza el valor del domicilio para una orden específica,
        modificando tanto la columna 'delivery_price' en 'orders.order_details'
        como el pe_json en 'orders.orders', y recalculando el total.
        """

        # 1) Obtener pe_json de la orden
        select_order_query = """
            SELECT pe_json
            FROM orders.orders
            WHERE id = %s;
        ...
        pe_json = result[0]  # pe_json se almacena en la primera columna devuelta

        # 2) Ajustar el pe_json
        #    - Restar el costo anterior de domicilio
        #    - Actualizar con el nuevo valor
        #    - Volver a sumar al total
        old_cost = pe_json["delivery"].get("delivery_costoenvio", 0)
        pe_json["delivery"]["delivery_pagocon"] -= old_cost
        pe_json["delivery"]["delivery_costoenvio"] = new_delivery_price
        pe_json["delivery"]["delivery_pagocon"] += new_delivery_price

        # 3) Actualizar el campo 'delivery_price' en order_details
        update_details_query = """
            UPDATE orders.order_details
            SET delivery_price = %s
            WHERE order_id = %s;
        """
        self.cursor.execute(update_details_query, (new_delivery_price, order_id))

        # 4) Guardar el nuevo pe_json en la tabla orders
        update_pe_json_query = """
            UPDATE orders.orders
            SET pe_json = %s
            WHERE id = %s;
        """
        ...
        print(f"El valor del domicilio para la orden {order_id} se ha actualizado a {new_delivery_price} correctamente.")
    ...
    def get_distrimonster_user_by_dni(self, dni):
    ...
    def create_distrimonster_usuario(self, user_data: User):
        
        query = """
        
        INSERT INTO public.users(
         user_name, user_phone, user_address, site_id, cedula_nit, email, first_last_name, second_last_name, second_name)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) returning *;
        """
        
        result = self.db.execute_query(query=query,params= [
            user_data.user_name,
            user_data.user_phone,
            user_data.user_address,
            user_data.site_id,
            user_data.cedula_nit,
            user_data.email,
            user_data.first_last_name,
    ...
    def get_orders_to_validate(self):
        ...
        combined_order_query = """
            ...
            WHERE 
                latest_status_timestamp >= %s 
                AND latest_status_timestamp < %s 
                  AND current_status = 'validacion pendiente' AND site_id = 32
            ORDER BY order_id, latest_status_timestamp DESC;
            """
        self.cursor.execute(combined_order_query, (today_start, tomorrow_start))
        orders_info = self.cursor.fetchall()
    ...
    def aprove_order(self, order_id, responsible_id):
        """
        Authorize an order and update the responsible person.
