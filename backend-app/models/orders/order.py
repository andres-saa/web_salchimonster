...
class Order:
    ...
    def select_all_orders(self):
        select_query = "SELECT * FROM orders.complete_order_view;"
        self.cursor.execute(select_query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
        
        
    def select_order_by_site_id(self,site_id:int):
        select_query = f""" SELECT * FROM orders.complete_order_view where site_id = {site_id}"""
        self.cursor.execute(select_query,(site_id))
        # columns = [desc[0] for desc in self.cursor.description]       
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
        
    def select_order_products_by_order_id(self, order_id: str):
        # Use %s as a placeholder for the order_id
        select_query = "SELECT * FROM orders.order_products WHERE order_id = %s"
        # Pass order_id as a tuple in the execute method
