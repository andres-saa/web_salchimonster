...
from schema.inventory.monthly_inventory import GroupmonthlyInventoryItems,monthlyInventory,monthlyInventoryItems
load_dotenv()
...
class monthlyInventory:
    ...
    def get_all_monthly_Inventory_reports (self):
        query = f""" select * from inventory.view_monthly_inventory_details;        
        """
    ...
    def get_all_monthly_Inventory_reports_by_responsible_id (self, responsible_id:int):
        query = f""" select * from inventory.view_monthly_inventory_details where responsible_id = {responsible_id};        
        """
        self.cursor.execute(query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
    ...
    def get_all_monthly_inventory_reports_by_responsible_id_filtered(self, responsible_id: int, start_date: str, end_date: str):
        query = f"""
    ...
    def get_all_monthly_inventory_reports_filtered(self, site_ids: list, start_date: str, end_date: str):
        if len(site_ids) == 1:
    ...
    def get_all_monthly_Inventory_entries (self,monthly_inventory_id):
        query = f""" select * from inventory.detailed_monthly_inventory_entries where monthly_inventory_id = {monthly_inventory_id};        
        """
        self.cursor.execute(query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
    ...
    def get_all_monthly_Inventory_item_by_group_name (self,monthly_inventory_group):
        query = f""" select * from inventory.complete_monthly_items where group_name = '{monthly_inventory_group}' and status = true ;        
        """
        self.cursor.execute(query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
    
    def get_all_monthly_group_items (self):
        query = f""" select * from inventory.group_monthly_inventory_items where status = true;        
        """
        self.cursor.execute(query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
    
    def get_all_monthly_group_unit_measures (self):
        query = f""" select * from inventory.monthly_inventory_unit_measures where status = true;        
        """
        self.cursor.execute(query)
        columns = [desc[0] for desc in self.cursor.description]
        return [dict(zip(columns, row)) for row in self.cursor.fetchall()]
    ...
    def insert_monthly_inventory_group(self, group_data: GroupmonthlyInventoryItems):
        insert_query = """
    ...
    def insert_monthly_inventory_item(self, item_data: monthlyInventoryItems):
        insert_query = """
        ...
        """
        # Como city_id es generado automáticamente, no necesitamos insertarlo manualmente
        self.cursor.execute(insert_query, (item_data.name,item_data.group_monthly_inventory_item_id,item_data.monthly_inventory_item_unit_measure_id))
        group_id = self.cursor.fetchone()[0]
        self.conn.commit()
        return group_id
    ...
    def disable_monthly_inventory_group(self, group_Id:int):
        insert_query = """
    ...
    def disable_monthly_inventory_item(self, item_id:int):
        insert_query = """
    ...
    def insert_complete_inventory(self, responsible_id, site_id, items_data):
        ...
        for item in items_data:
            self.cursor.execute(insert_entry_query, (
                inventory_id,
                item.monthly_inventory_item_id,  # Asumiendo que los datos vienen en un diccionario
                item.quantity,
                item.monthly_inventory_unit_measure_id
            ))
        self.conn.commit()
