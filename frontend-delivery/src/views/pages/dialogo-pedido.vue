<template>
    <Dialog class="dialogo" style=" border-radius: 1rem; overflow: ; background-color: rgb(255, 255, 255); color: black"
        ...
        <div style="display:  flex; flex-direction: column; width: 100%; align-items: center; gap: 20px;">
            <Textarea v-model="razon" style="width: 100%; min-height: 10rem;" placeholder="Razon por la cual cancelara el pedido" ></Textarea>

         
            <div style="display: flex; gap: 1rem;">
              
                <Button severity="danger" outlined @click=" () => {cancelarPedido(pedido,razon)} "> CANCELAR PEDIDO</Button>
                <Button severity="danger" @click="visibleCancelar = false"> CERRAR</Button>
    ...
    <Dialog class="dialogo"
        ...
        <div class="btn_accion_dialogo" style="position: absolute; bottom:1rem; display: flex; gap: 2rem">
            ...
            <button v-if="pedido.order_status.status == 'generada' && pedido.order_status.status != 'enviada'"
                @click="prepararPedido(pedido)"
                style="border: none;box-shadow: 0 0 1rem rgba(0, 0, 0, 0.5); padding: 0rem 2rem;border-radius: 0.5rem; background-color: rgb(0, 255, 106);"><span
                    class="text-xl p-0" style="border: none; font-weight: bold; ">PREPARAR</span>
            </button>

            <button v-if="pedido.order_status.status == 'en preparacion' && pedido.order_status.status != 'enviada'"
                @click="marcarComoEnviado(pedido)"
                style="border: none;box-shadow: 0 0 1rem rgba(0, 0, 0, 0.5); padding: 0rem 2rem;border-radius: 0.5rem; background-color: rgb(0, 255, 0);"><span
                    class="text-xl p-0" style="border: none; font-weight: bold; ">ENVIAR</span>
            </button>

            <button @click="showCancelar()" v-if="pedido.order_status.status != 'enviada'"
                style="border: none;box-shadow: 0 0 1rem rgba(0, 0, 0, 0.5); padding: 0.5rem 1rem;border-radius: 0.5rem; background-color: rgb(255, 120, 120);"><span
                    class="text-xl" style="border: none; font-weight: bold; ">CANCELAR</span>
            </button>

            <button @click="IMPRIMIR"
                style="border: none;box-shadow: 0 0 1rem rgba(0, 0, 0, 0.5); padding: 0.5rem 2rem;border-radius: 0.5rem; background-color: rgb(255, 246, 120);"><span
                    class="text-xl p-0" style="border: none; font-weight: bold; ">IMPRIMIR</span>
            </button>
...
import { prepararPedido, marcarComoEnviado, cancelarPedido, visibleCancelar } from '../../service/un_pedido';
import factura from './factura.vue';

const razon = ref('El encargado no dio razones')



const showCancelar = () => {
    visibleCancelar.value = true
}
