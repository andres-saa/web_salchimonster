    ...
    <div class="col-12 " style="">
        <div class="   mt-4 p-4   "
                    ...
                    <div class="producto" v-for="(product,index) in products"
                        ...
                        <div v-if="product.product.adiciones?.length > 0 || product.product.toppings?.length > 0 || product.product.acompanantes?.length > 0 || product.product.cambios?.length > 0 " class=" p-0 col-12 " style="display: flex; justify-content: space-between;border-radius: ; ">
                            <p class="text-left   m-0 " style="font-weight: bold;height: ;text-transform: ;"><b style=" font-weight: bold;">TOTAL</b> {{ product.quantity }} {{ product.product.name }}  {{product.product.adiciones?.length > 1 ? ' + adiciones': '' }} 
                                ...
                                <p class="text-right text-md  m-0 text-l  bold" style=" font-weight: bold;height: ;">{{ formatoPesosColombianos(product.quantity *
                ...
                <div class="text-xl p-0 col-12 my-0 " style="; width: 100%; display: flex; justify-content: space-between; margin: 1rem 0;">
                    ...
                    <span style="font-weight: bold; text-transform: ;"> Domicilio a  <span 
                            style="margin-right: 1rem; cursor: pointer; color: var(--primary-color);"><span class="domi-name">{{ thanks_data.user_data.user_address
                            }}</span> </span> </span>
                    <span class="bold" style="font-weight: bold;"> {{ formatoPesosColombianos(thanks_data.delivery_price)
                    }}
                    </span>
                ...
                id=""> -->
                <p  style=" min-height:5rem ;margin-bottom: 2rem; border;" class="col-12 "
                    type="text" name="" id="dynamicTextarea"> {{ thanks_data.order_notes }}</p>
...
import { domicilio,contarObjetosRepetidos} from '../../service/cart';

import { calcularPrecioTotal, calcularTotalCarrito} from '../../service/state';

import {order_notes, thanks_data, user_data} from '../../service/order';
...
const total = ref()
onMounted(() => {
    products.value = contarObjetosRepetidos(thanks_data.value.order_products) ;
